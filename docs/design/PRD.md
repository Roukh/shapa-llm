...
- The `shapa` CLI (`cli.py`) incl. `init` — scaffolds a `shapa/` wiki (default `./shapa`, a repo-root wiki), installs `AGENTS.md` + generic `arch/` project templates, and records the path; the external-wiki + **cwd-discovery** resolution model (`config.py`: a session inside a repo resolves that repo's `shapa/` wiki automatically). Test suite + fixtures. `install.sh` + `bootstrap.sh` (curl|sh) wire the fetch/capture/maintain hooks and scaffold the default global wiki.
